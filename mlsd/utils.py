...
def pred_lines(image, model,
    ...
    if device.type == 'cpu':
        batch_image = torch.from_numpy(batch_image).float().cpu()
    else:
...
_default_params = PredSquaresParams(score=0.06,
                                    outside_ratio=0.28,
                                    inside_ratio=0.45,
                                    w_overlap=0.0,
                                    w_degree=1.95,
...
def pred_squares(image,
    ...
    '''
    shape = [height, width]
    new_segments, squares, score_array, inter_points
    '''
    h, w, _ = image.shape
    original_shape = [h, w]
